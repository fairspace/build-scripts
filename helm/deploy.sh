...
helm upgrade --install "${APPNAME}-${ENVIRONMENT}" "fairspace/${APPNAME}" --namespace "${APPNAME}-${ENVIRONMENT}" --version "$VERSION" -f "${DEPLOYMENT_CONFIG_DIR}/${ENVIRONMENT}-values.yaml" --timeout "1500s" --history-max 1 $EXTRA_ARGS
